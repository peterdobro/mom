...
!dhb599:                                swflx, sw_frac_zt, pme, river, visc_cbu, diff_cbt, hblt_depth, do_wave)
                                swflx, sw_frac_zt, pme, river, visc_cbu, diff_cbt, diff_cbt_conv, hblt_depth, Ice_ocean_boundary, do_wave)
...
!
  ...
  if ( .not. module_is_initialized ) then 
     call mpp_error(FATAL, '==>Error from ocean_vert_kpp: module must be initialized')
  endif 
...
!-----------------------------------------------------------------------
      ...
      call enhance(Thickness, diff_cbt, visc_cbu) 
...
!-----------------------------------------------------------------------
      ...
      ! compute viscosity on "U-grid" by averaging visc_cbu from "T-grid". 
...
!      diff_cbt = diffusion coefficient at bottom of "t" cells (m**2/s)         <BR/>  diff_cbt
! </DESCRIPTION>
!
...
!-----------------------------------------------------------------------
            ...
            fconu  = 0.5 * ( abs(riu(i,j,k)) - riu(i,j,k) )
...
!-----------------------------------------------------------------------
          ...
          dvdzup = (visc_cbu(i,j,knm1) - visc_cbu(i,j,kn))/Thickness%dzt(i,j,kn)
          dvdzdn = (visc_cbu(i,j,kn) - visc_cbu(i,j,knp1))/Thickness%dzt(i,j,knp1)
...
!
! </DESCRIPTION>
subroutine enhance(Thickness, diff_cbt, visc_cbu)
